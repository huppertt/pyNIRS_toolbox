...
class Statistics:
    ...
    def table(self):
        ...
        return pd.DataFrame( {'Channel':self.betas.channels,'Type':self.betas.type,'Condition':self.betas.conditions,
                    'Beta':self.betas,'StdErr':StdErr,'T-value':t_values,'P-values':p_values,'Q-values':q_values})
            ...
            for cc in contrast:
                ...
                for typ in types:
                    for key, value in localcontrast.items():
                        c[cnt,(self.betas.channels==chan) & (self.betas.type==typ) & (self.betas.conditions==key)]=value
                    cnt+=1
