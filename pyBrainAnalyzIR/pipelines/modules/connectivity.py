...
from pyBrainAnalyzIR.dataclasses.options_variables import (
    OptionsDict, NumericOption, BooleanOption, QuantityOption, StringOption)
...
class resting_state_connectivity(cedalion_module):
    ...
    def __init__(self, previous_job=None):
        ...
        self._cite = "Santosa, Hendrik, et al. 'Characterization and correction of the false-discovery rates in resting state connectivity using functional near-infrared spectroscopy.' Journal of biomedical optics 22.5 (2017): 055002-055002."
    ...
    def __init__(self, previous_job=None):
        ...
        self.options = OptionsDict({
            ...
            'robust': BooleanOption(True,
                                       description='Robust estimator',
                                                 help='If True, a robust version of correlation will be used.'),
            'grouping_variable': StringOption('pair', description='Dyad grouping variable',
                                                   help='The name of the variable in the demographics that indicates which subjects are in a dyad. This is used to compute hyperscanning connectivity between subjects in the same dyad.'),
            'unordered': BooleanOption(False,
                                    description='Treat as unordered pairs',
                                    help='if false, then the order of the subjects in the dyad will be used to determine which subject is "from" and which is "to". If true, then the subjects will be treated as unordered pairs.'),
    ...
    def _runlocal(self, dset):
        ...
        for upair in upairs:
            ...
            if len(lst) >1:
                ...
                df=compute_hyperscanning(dyad, robust=self.options['robust'], AR=self.options['AR'], unordered=self.options['unordered'])
