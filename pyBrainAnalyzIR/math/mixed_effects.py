...
def fitlme(X, Y, Z, robust_flag=False, zero_theta=False, verbose=False):
    ...
    # Solve initial model
    LL_val, beta_val, bHat_val, covb_val, _sigma2 = solveLME(X, Y, Z, theta)
    ...
    if robust_flag:
        ...
        # Calculate sigma
        # NOTE: this branch only runs when robust_flag is True, so the
        # non-robust blend of sigma2/sigma_robust from the original MATLAB
        # implementation is unreachable here and has been removed.
...
def solveLME(X, Y, Z, theta=0, weights=None):
    ...
    nT, _nY = Y.shape if Y.ndim > 1 else (Y.shape[0], 1)
    if Y.ndim == 1:
        Y = Y.reshape(-1, 1)
    nX = X.shape[1]
...
def cholSafe(d, mode='upper'):
    ...
    while p != 0:
        try:
            if mode == 'lower':
                R = cholesky(d + delta * eye, lower=True)
            else:
                R = cholesky(d + delta * eye, lower=False)
            p = 0
...
def chol_permuted(A):
    ...
    try:
        ...
        eye = np.eye(A_dense.shape[0])
        R = cholSafe(A_dense + delta * eye, mode='upper')
        p = 0
        S = np.eye(A_dense.shape[0])
        return sparse.csr_matrix(R), p, S
