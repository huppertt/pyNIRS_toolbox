...
"""# BIDS Dataset
This dataset is organized according to the Brain Imaging Data Structure (BIDS) standard. For more information about BIDS, please visit [https://bids.neuroimaging.io/](https://bids.neuroimaging.io/).
"""
...
def write_bids_readme(path: str | Path, description: dict):
    ...
    key_order=['Overview', 'Experimental design ', 'Dataset contents and structure', 'Methods', 'Participants and recruitment', 'Acquisition', 'Task and paradigm', 'Stimuli ', 'Additional data acquired ', 'Known issues, quality, and missing data', 'How to use these data', 'References and citation', 'Contact']
...
def add_missing_bids_to_metadata(data,intype='amp'):
    ...
    if (data.__class__ == pyBrainAnalyzIR.dataclasses.dataset.DataSet):
        ...
        defaults={'Manufacturer':'n/a',
                ...
                'NIRSSourceOptodeCount': sum((data.geo2d.type==PointType(1).SOURCE).values==True),
                'NIRSDetectorOptodeCount': sum((data.geo2d.type==PointType(1).DETECTOR).values==True),
                'ShortChannelCount':sum((xrutils.norm(data.geo3d.loc[data[intype].source] - data.geo3d.loc[data[intype].detector], data.geo3d.points.crs)<units.millimeter*15).values==True),
                }
